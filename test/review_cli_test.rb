...
class ReviewCLITest < Minitest::Test
  def test_parses_repos_with_defaults
    options = BasecampAgentConnector::GitHub::ReviewCLI.parse_options([ "basecamp/bc3" ])
  ...
  def test_parses_multiple_repos_and_flags
    options = BasecampAgentConnector::GitHub::ReviewCLI.parse_options([ "acme/a", "acme/b", "--events", "pull_request_review,issue_comment", "--port", "4567" ])
  ...
  def test_requires_a_repo
    assert_raises ArgumentError do
      BasecampAgentConnector::GitHub::ReviewCLI.parse_options([ "--port", "4567" ])
    end
  end
end
